...
-- Remover funções auth (se existirem)
DROP FUNCTION IF EXISTS auth.user_cargo() CASCADE;
DROP FUNCTION IF EXISTS auth.user_regional() CASCADE;
DROP FUNCTION IF EXISTS auth.can_access(TEXT) CASCADE;
...
CREATE OR REPLACE FUNCTION auth.can_access(required_cargo TEXT)
RETURNS BOOLEAN AS $$
DECLARE
    user_cargo TEXT;
    user_nivel INTEGER;
...
CREATE POLICY "Veículos por regional - write" ON veiculos
    FOR ALL USING (
        (regional_id = auth.user_regional() AND auth.can_access('SUPERVISOR')) OR 
        auth.user_cargo() = 'DIRETOR'
    );
...
CREATE POLICY "OS por regional - write" ON ordens_servico
    FOR ALL USING (
        EXISTS (
            ...
            WHERE v.id = ordens_servico.veiculo_id 
            AND (v.regional_id = auth.user_regional() OR auth.user_cargo() = 'DIRETOR')
        ) AND auth.can_access('SUPERVISOR')
    );
...
CREATE POLICY "Peças por regional - write" ON pecas
    FOR ALL USING (
        (regional_id = auth.user_regional() AND auth.can_access('ANALISTA_OFICINA')) OR 
        auth.user_cargo() = 'DIRETOR'
    );
...
CREATE POLICY "Serviços externos por regional - write" ON servicos_externos
    FOR ALL USING (
        EXISTS (
            ...
            WHERE os.id = servicos_externos.ordem_servico_id
            AND (v.regional_id = auth.user_regional() OR auth.user_cargo() = 'DIRETOR')
        ) AND auth.can_access('ANALISTA_OFICINA')
    );
...
SELECT 'Funções auth criadas:' as info;
SELECT proname FROM pg_proc WHERE pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'auth') AND proname LIKE 'user_%' OR proname = 'can_access';
